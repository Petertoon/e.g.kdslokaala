...
app.get("/api/table-order-check/:locationId", requireDb, async (req, res) => {
  try {
    const [allowed, { rows: settingsRows }] = await Promise.all([
      ...
      pool.query(`SELECT ordering_enabled, primary_language FROM kiosk_settings WHERE location_id = $1`, [req.params.locationId]),
    ]);
    ...
    res.json({ allowed, orderingEnabled, primaryLanguage });
  } catch (err) {
...
app.put("/api/admin/kiosk-settings/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO kiosk_settings (location_id, idle_timeout_seconds, ad_interval_seconds, header_button_image_url, header_button_link_url, ordering_enabled, primary_language)
     VALUES ($1,$2,$3,$4,$5,$6,$7)
     ON CONFLICT (location_id) DO UPDATE SET
       idle_timeout_seconds=$2, ad_interval_seconds=$3, header_button_image_url=$4, header_button_link_url=$5, ordering_enabled=$6, primary_language=$7
     RETURNING *`,
    [req.params.locationId, idleTimeoutSeconds, adIntervalSeconds, headerButtonImageUrl, headerButtonLinkUrl, orderingEnabled, primaryLanguage]
  );
  res.json(rows[0]);
});
