...
app.put("/api/admin/kiosk-settings/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  const idleTimeoutSeconds = Number(req.body.idleTimeoutSeconds) || 60;
  const adIntervalSeconds = Number(req.body.adIntervalSeconds) || 325;
  const { rows } = await pool.query(
    `INSERT INTO kiosk_settings (location_id, idle_timeout_seconds, ad_interval_seconds)
     VALUES ($1,$2,$3)
     ON CONFLICT (location_id) DO UPDATE SET idle_timeout_seconds=$2, ad_interval_seconds=$3
     RETURNING *`,
    [req.params.locationId, idleTimeoutSeconds, adIntervalSeconds]
  );
  res.json(rows[0]);
});
...
app.post("/api/kiosk-order", requireDb, express.json(), async (req, res) => {
  try {
    const { locationId, customerName, orderType, items } = req.body;
    ...
    const order = {
      ...
      readyTime: null,
      fulfillmentMethod: `${orderTypeLabel} — pay at counter`,
      items: orderItems,
      notes: null,
