...
app.post("/api/admin/menu/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO menu_items (location_id, category, name, price, description, modifiers, active, sort_order, image_url, name_alt, description_alt)
...
app.put("/api/admin/menu/item/:id", requireDb, requireAdmin, express.json(), async (req, res) => {
  const { category, name, price, description, modifiers, active, sortOrder, imageUrl, locationId } = req.body;
  const finalCategory = category || "Menu";
  ...
  const { rows } = await pool.query(
    `UPDATE menu_items SET category=$1, name=$2, price=$3, description=$4, modifiers=$5, active=$6, sort_order=$7, image_url=$8, name_alt=$9, description_alt=$10
     WHERE id=$11 RETURNING *`,
    [
      finalCategory, name, price ?? null, description || null, JSON.stringify(translatedModifiers),
      active !== false, sortOrder || 0, imageUrl || null, nameAlt, descriptionAlt, req.params.id,
    ]
  );
