...
app.get("/api/category-ads/:locationId", requireDb, async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT category, headline, highlight, body, image_url, button_text, button_link
...
app.put("/api/admin/category-ads/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  const { category, headline, highlight, body, imageUrl, buttonText, buttonLink } = req.body;
  if (!category) return res.status(400).json({ error: "category is required" });
  const { rows } = await pool.query(
    `INSERT INTO category_ads (location_id, category, headline, highlight, body, image_url, button_text, button_link)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
     ON CONFLICT (location_id, category) DO UPDATE SET
       headline=$3, highlight=$4, body=$5, image_url=$6, button_text=$7, button_link=$8
     RETURNING *`,
    [req.params.locationId, category, headline || null, highlight || null, body || null, imageUrl || null, buttonText || null, buttonLink || null]
  );
  res.json(rows[0]);
});
