...
// Separate socket just for the waitress station screen — kept apart from
// /kds-stream (food orders) so this alerts-only screen doesn't need to know
// anything about order tickets, and vice versa.
const stationWss = new WebSocketServer({ server, path: "/station-stream" });

const PORT = process.env.PORT || 3000;
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || null;
// Separate from ADMIN_PASSWORD on purpose — servers/waitstaff get a password
// that only unlocks the service-request alert screen, not menu editing, QR
// codes, or anything else in /admin.
const STATION_PASSWORD = process.env.STATION_PASSWORD || null;
...
async function initDb() {
  ...
  // Lets an item be hidden from the To-Go / pickup ordering page while still
  // showing on the kiosk and table menus. Defaults true so nothing disappears
  // from the new To-Go menu until someone actually unchecks it in admin.
  await pool.query(`ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS available_togo BOOLEAN DEFAULT true;`);
  // Auto-translated (via DeepL) counterpart of name/description, in whichever
  // language ISN'T the location's primary_language. Populated automatically
  // when an item is saved in admin — see translateAndAttach() below.
  await pool.query(`ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS name_alt TEXT;`);
  await pool.query(`ALTER TABLE menu_items ADD COLUMN IF NOT EXISTS description_alt TEXT;`);
  ...
  // Master switch for locations that don't want a KDS/printer pipeline at
  // all — just QR menu browsing. Defaults true so every existing location
  // keeps ordering exactly as it works today.
  await pool.query(`ALTER TABLE kiosk_settings ADD COLUMN IF NOT EXISTS ordering_enabled BOOLEAN DEFAULT true;`);
  // 'en' or 'es' — the language menu items are typed in for this location.
  // The auto-translated "_alt" fields hold the other language.
  await pool.query(`ALTER TABLE kiosk_settings ADD COLUMN IF NOT EXISTS primary_language TEXT DEFAULT 'en';`);
  // Defaults true so the service-request button stays on for every existing
  // location unless an owner actively turns it off for that one.
  await pool.query(`ALTER TABLE kiosk_settings ADD COLUMN IF NOT EXISTS service_requests_enabled BOOLEAN DEFAULT true;`);

  // Auto-translated category name (the tab label in the other language).
  // Filled in the first time a menu item in that category is saved;
  // editable afterward from the admin panel like everything else here.
  await pool.query(`
  ...
  `);
  // Added to support three selectable banner layouts (see table-order.html):
  // 1 = hero photo + badges + text bar, 2 = hero photo + feature-icon row, 3 = side card (original/default).
  await pool.query(`ALTER TABLE category_ads ADD COLUMN IF NOT EXISTS template INTEGER DEFAULT 3;`);
...
// -----------------------------------------------------------------------------
// Waitress station — a separate, password-gated screen (private/waitress-
// station.html) that shows live service-request alerts and lets staff
// acknowledge and complete them. Kept on its own WebSocket path and its own
// in-memory store, entirely decoupled from food orders and the KDS board.
// Memory-only by design (same tradeoff as knownLocations) — a request is a
// "someone needs help right now" alert, not a record worth surviving a
// restart; if the server redeploys mid-shift, a customer would just tap the
// bell again.
// -----------------------------------------------------------------------------
const stationsByLocation = new Map(); // locationId -> Set<ws>
const serviceRequestsByLocation = new Map(); // locationId -> Map<requestId, request>

function addStation(locationId, ws) {
  if (!stationsByLocation.has(locationId)) stationsByLocation.set(locationId, new Set());
  stationsByLocation.get(locationId).add(ws);
}
function removeStation(locationId, ws) {
  stationsByLocation.get(locationId)?.delete(ws);
}

stationWss.on("connection", (ws, req) => {
  const url = new URL(req.url, "http://localhost");
  const locationId = url.searchParams.get("location") || "unassigned";
  ws.locationId = locationId;
  addStation(locationId, ws);
  console.log(`[station] screen connected for location "${locationId}"`);

  // Send whatever's currently pending/acknowledged so a station that just
  // opened (or reconnected after a network blip) isn't starting blind.
  const current = [...(serviceRequestsByLocation.get(locationId)?.values() || [])];
  if (locationId === "all") {
    for (const reqsForLoc of serviceRequestsByLocation.values()) current.push(...reqsForLoc.values());
  }
  ws.send(JSON.stringify({ type: "snapshot", requests: current }));

  ws.on("close", () => {
    removeStation(locationId, ws);
    console.log(`[station] screen disconnected for location "${locationId}"`);
  });
});

function broadcastToStations(locationId, payload) {
  const json = JSON.stringify(payload);
  let delivered = 0;
  for (const client of stationsByLocation.get(locationId) || []) {
    if (client.readyState === client.OPEN) { client.send(json); delivered++; }
  }
  for (const client of stationsByLocation.get("all") || []) {
    if (client.readyState === client.OPEN) { client.send(json); delivered++; }
  }
  return delivered;
}
...
// Same delivery mechanism as broadcastOrder(), but for a customer flagging
// down staff from table-order.html rather than placing food — delivered to
// the waitress station screen(s) for this location, and kept in memory so
// staff can acknowledge/complete it (see the endpoints near /api/kiosk-order).
function broadcastServiceRequest(request) {
  if (!serviceRequestsByLocation.has(request.locationId)) {
    serviceRequestsByLocation.set(request.locationId, new Map());
  }
  serviceRequestsByLocation.get(request.locationId).set(request.id, request);

  const delivered = broadcastToStations(request.locationId, { type: "service_request", request });
  console.log(
    `[service-request] Table ${request.tableNumber} (location: ${request.locationId}) delivered to ${delivered} station(s)`
  );
}
...
// Same pattern as requireAdmin, but its own password — waitstaff get in to
// see and acknowledge service-request alerts without touching /admin at all.
function requireStation(req, res, next) {
  if (!STATION_PASSWORD) {
    return res.status(503).send("Set STATION_PASSWORD in your environment variables to enable the waitress station screen.");
  }
  const auth = req.headers.authorization;
  if (auth && auth.startsWith("Basic ")) {
    const [, password] = Buffer.from(auth.slice(6), "base64").toString().split(":");
    if (password === STATION_PASSWORD) return next();
  }
  res.set("WWW-Authenticate", 'Basic realm="Waitress Station"');
  return res.status(401).send("Authentication required.");
}
...
// ---- DeepL translation (menu EN/ES toggle) ---------------------------------
// Translation is entirely optional — every function here degrades to "do
// nothing" if DEEPL_API_KEY isn't set, so menu saving/serving never breaks
// because of it.
function deeplLangCodes(primaryLanguage) {
  // DeepL wants a region on English as a TARGET (EN-US), but accepts plain
  // EN as a SOURCE — this mapping keeps that straight in both directions.
  return primaryLanguage === "es"
    ? { sourceLang: "ES", targetLang: "EN-US" }
    : { sourceLang: "EN", targetLang: "ES" };
}

// Translates a batch of strings in one API call (cheaper and much faster
// than one call per field). Returns an array of translations in the same
// order, or null if translation isn't configured / the call failed —
// callers must treat null as "skip translation," not as an error to surface.
async function deeplTranslateBatch(texts, primaryLanguage) {
  const key = process.env.DEEPL_API_KEY;
  if (!key || !texts.length) return null;
  const { sourceLang, targetLang } = deeplLangCodes(primaryLanguage);
  const base = key.endsWith(":fx") ? "https://api-free.deepl.com" : "https://api.deepl.com";
  try {
    const res = await fetch(`${base}/v2/translate`, {
      method: "POST",
      headers: { Authorization: `DeepL-Auth-Key ${key}`, "Content-Type": "application/json" },
      body: JSON.stringify({ text: texts, source_lang: sourceLang, target_lang: targetLang }),
    });
    if (!res.ok) {
...
async function getPrimaryLanguage(locationId) {
  const { rows } = await pool.query(`SELECT primary_language FROM kiosk_settings WHERE location_id = $1`, [locationId]);
  return rows[0]?.primary_language || "en";
}

// Translates a menu item's name/description/modifier names in one batch
// call, and — the first time a given category is seen — its category name
// too. Mutates and returns { nameAlt, descriptionAlt, modifiers } for the
// caller to save; never throws (translation failures just leave *_alt null).
async function translateMenuItem(locationId, { name, description, category, modifiers }) {
  const primaryLanguage = await getPrimaryLanguage(locationId);
  const texts = [name, description || ""];
  const modifierList = modifiers || [];
  modifierList.forEach((m) => texts.push(m.name || ""));

  const { rows: existingCat } = await pool.query(
    `SELECT 1 FROM category_translations WHERE location_id = $1 AND category = $2`,
    [locationId, category]
  );
  const needsCategoryTranslation = category && !existingCat.length;
  if (needsCategoryTranslation) texts.push(category);

  const translated = await deeplTranslateBatch(texts, primaryLanguage);
  if (!translated) return { nameAlt: null, descriptionAlt: null, modifiers: modifierList };

  const nameAlt = translated[0] || null;
  const descriptionAlt = translated[1] || null;
  const newModifiers = modifierList.map((m, i) => ({ ...m, nameAlt: translated[2 + i] || null }));

  if (needsCategoryTranslation) {
    const categoryAlt = translated[translated.length - 1];
    await pool.query(
      `INSERT INTO category_translations (location_id, category, translated) VALUES ($1,$2,$3)
       ON CONFLICT (location_id, category) DO UPDATE SET translated = $3`,
      [locationId, category, categoryAlt]
    );
  }

  return { nameAlt, descriptionAlt, modifiers: newModifiers };
}
...
// Pass ?togo=true to get only the subset checked "Available on To-Go menu"
// in admin — used by togo-order.html so drive-thru/pickup customers only
// see items the kitchen has approved for that channel.
app.get("/api/menu/:locationId", requireDb, async (req, res) => {
  try {
    const togoOnly = req.query.togo === "true";
    const { rows } = await pool.query(
      `SELECT id, category, name, price, description, modifiers, image_url, name_alt, description_alt
       FROM menu_items WHERE location_id = $1 AND active = true ${togoOnly ? "AND available_togo = true" : ""}
    ...
    res.json(
      rows.map((r) => ({
        ...
        nameAlt: r.name_alt,
        name_alt: undefined,
        descriptionAlt: r.description_alt,
        description_alt: undefined,
      }))
    );
  } catch (err) {
...
app.get("/api/kiosk-config/:locationId", requireDb, async (req, res) => {
  try {
    const [{ rows: ads }, { rows: settingsRows }] = await Promise.all([
      ...
      pool.query(`SELECT idle_timeout_seconds, ad_interval_seconds, ordering_enabled, primary_language FROM kiosk_settings WHERE location_id = $1`, [
        req.params.locationId,
      ]),
    ]);
    ...
    res.json({
      ...
      primaryLanguage: settings.primary_language || "en",
    });
  } catch (err) {
...
// ---- Public: translated category (tab) names for this location -----------
app.get("/api/category-translations/:locationId", requireDb, async (req, res) => {
  try {
    const { rows } = await pool.query(`SELECT category, translated FROM category_translations WHERE location_id = $1`, [
      req.params.locationId,
    ]);
    const map = {};
    rows.forEach((r) => (map[r.category] = r.translated));
    res.json(map);
  } catch (err) {
    console.error("[category-translations] fetch failed:", err.message);
    res.status(500).json({ error: "Could not load category translations" });
  }
});
...
app.get("/api/category-ads/:locationId", requireDb, async (req, res) => {
  try {
    ...
    console.error("[category-ads] fetch failed:", err.message);
    res.status(500).json({ error: "Could not load category ads" });
  }
});
...
app.get("/api/table-order-check/:locationId", requireDb, async (req, res) => {
  try {
    const [allowed, { rows: settingsRows }] = await Promise.all([
      isOnApprovedNetwork(req.params.locationId, getClientIp(req)),
      pool.query(`SELECT ordering_enabled, primary_language, service_requests_enabled FROM kiosk_settings WHERE location_id = $1`, [req.params.locationId]),
    ]);
    const orderingEnabled = settingsRows[0] ? settingsRows[0].ordering_enabled !== false : true;
    const primaryLanguage = settingsRows[0]?.primary_language || "en";
    const serviceRequestsEnabled = settingsRows[0] ? settingsRows[0].service_requests_enabled !== false : true;
    res.json({ allowed, orderingEnabled, primaryLanguage, serviceRequestsEnabled });
  } catch (err) {
...
app.post("/api/admin/menu/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  const { category, name, price, description, modifiers, active, sortOrder, imageUrl, availableTogo } = req.body;
  if (!name) return res.status(400).json({ error: "name is required" });
  const finalCategory = category || "Menu";
  const { nameAlt, descriptionAlt, modifiers: translatedModifiers } = await translateMenuItem(req.params.locationId, {
    name,
    description,
  ...
  const { rows } = await pool.query(
    `INSERT INTO menu_items (location_id, category, name, price, description, modifiers, active, sort_order, image_url, available_togo, name_alt, description_alt)
    ...
    [
      ...
      price ?? null,
      description || null,
      JSON.stringify(translatedModifiers),
      active !== false,
      sortOrder || 0,
      imageUrl || null,
      availableTogo !== false,
...
app.put("/api/admin/menu/item/:id", requireDb, requireAdmin, express.json(), async (req, res) => {
  const { category, name, price, description, modifiers, active, sortOrder, imageUrl, availableTogo, locationId } = req.body;
  const finalCategory = category || "Menu";
  // locationId isn't stored per-edit (it never changes), but translation needs
  // to know which location's primary_language to translate against — the
  // admin panel always has it handy from the currently-loaded location.
  const { nameAlt, descriptionAlt, modifiers: translatedModifiers } = locationId
    ? await translateMenuItem(locationId, { name, description, category: finalCategory, modifiers })
    : { nameAlt: null, descriptionAlt: null, modifiers: modifiers || [] };
  const { rows } = await pool.query(
    `UPDATE menu_items SET category=$1, name=$2, price=$3, description=$4, modifiers=$5, active=$6, sort_order=$7, image_url=$8, available_togo=$9, name_alt=$10, description_alt=$11
     WHERE id=$12 RETURNING *`,
    [
      finalCategory, name, price ?? null, description || null, JSON.stringify(translatedModifiers),
      active !== false, sortOrder || 0, imageUrl || null, availableTogo !== false, nameAlt, descriptionAlt, req.params.id,
    ]
  );
...
app.put("/api/admin/kiosk-settings/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  ...
  const orderingEnabled = req.body.orderingEnabled !== undefined ? !!req.body.orderingEnabled : (existing.ordering_enabled !== false);
  const primaryLanguage = req.body.primaryLanguage !== undefined ? req.body.primaryLanguage : (existing.primary_language || "en");
  const serviceRequestsEnabled = req.body.serviceRequestsEnabled !== undefined ? !!req.body.serviceRequestsEnabled : (existing.service_requests_enabled !== false);
  const { rows } = await pool.query(
    `INSERT INTO kiosk_settings (location_id, idle_timeout_seconds, ad_interval_seconds, header_button_image_url, header_button_link_url, ordering_enabled, primary_language, service_requests_enabled)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
     ON CONFLICT (location_id) DO UPDATE SET
       idle_timeout_seconds=$2, ad_interval_seconds=$3, header_button_image_url=$4, header_button_link_url=$5, ordering_enabled=$6, primary_language=$7, service_requests_enabled=$8
     RETURNING *`,
    [req.params.locationId, idleTimeoutSeconds, adIntervalSeconds, headerButtonImageUrl, headerButtonLinkUrl, orderingEnabled, primaryLanguage, serviceRequestsEnabled]
  );
  res.json(rows[0]);
});
...
// ---- Admin: category translations (protected) ------------------------------
// Auto-filled by translateMenuItem() the first time a category is used;
// these routes just let the admin see/override the result.
app.get("/api/admin/category-translations/:locationId", requireDb, requireAdmin, async (req, res) => {
  const { rows } = await pool.query(`SELECT * FROM category_translations WHERE location_id = $1`, [req.params.locationId]);
  res.json(rows);
});

app.put("/api/admin/category-translations/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  const { category, translated } = req.body;
  if (!category) return res.status(400).json({ error: "category is required" });
  const { rows } = await pool.query(
    `INSERT INTO category_translations (location_id, category, translated) VALUES ($1,$2,$3)
     ON CONFLICT (location_id, category) DO UPDATE SET translated = $3 RETURNING *`,
    [req.params.locationId, category, translated || null]
  );
  res.json(rows[0]);
});

// ---- Admin: category ads / promo banner (protected) ------------------------
app.get("/api/admin/category-ads/:locationId", requireDb, requireAdmin, async (req, res) => {
  const { rows } = await pool.query(`SELECT * FROM category_ads WHERE location_id = $1`, [req.params.locationId]);
  res.json(rows);
});
...
app.post("/api/kiosk-order", requireDb, express.json(), async (req, res) => {
  try {
    const { locationId, customerName, customerPhone, orderType, channel, tableNumber, items } = req.body;
    ...
    // Locations set up as menu-viewing-only (no KDS/printer) never accept
    // orders through this endpoint, regardless of which page it came from.
    const { rows: settingsRows } = await pool.query(`SELECT ordering_enabled FROM kiosk_settings WHERE location_id = $1`, [locationId]);
    if (settingsRows[0] && settingsRows[0].ordering_enabled === false) {
      return res.status(403).json({ error: "Ordering is not available for this location." });
    }
    ...
    // To-Go / drive-thru orders here are on-site (customer's own phone while
    // parked at the drive-thru or waiting at pickup, on the restaurant's
    // WiFi) — same network check as table orders. This is separate from
    // off-site/delivery ordering, which still goes through the Wix
    // Restaurants storefront, not this page.
    if (channel === "togo") {
      if (!customerName || !customerPhone) {
        return res.status(400).json({ error: "customerName and customerPhone are required for To-Go orders" });
      }
      const allowed = await isOnApprovedNetwork(locationId, getClientIp(req));
      if (!allowed) {
        return res.status(403).json({ error: "Please connect to the restaurant's WiFi to place a To-Go order." });
      }
    }
    ...
    const fulfillmentMethod =
      channel === "table" ? `Table ${tableNumber} — Dine In`
      : channel === "togo" ? `To-Go / Pickup — ${customerPhone}`
      : `${orderTypeLabel} — pay at counter`;
    const order = {
      ...
      customerPhone: channel === "togo" ? customerPhone : null,
      deliveryType: null,
    };
...
// A customer flagging down staff from table-order.html — not a food order,
// just a "come to this table" alert pushed to the KDS board over the same
// WebSocket. Re-checks the WiFi allowlist server-side, same as table orders,
// so this can't be triggered from outside the restaurant either.
app.post("/api/service-request", requireDb, express.json(), async (req, res) => {
  try {
    const { locationId, tableNumber, customerName } = req.body;
    if (!locationId || !tableNumber) {
      return res.status(400).json({ error: "locationId and tableNumber are required" });
    }
    const allowed = await isOnApprovedNetwork(locationId, getClientIp(req));
    if (!allowed) {
      return res.status(403).json({ error: "Please connect to the restaurant's WiFi." });
    }
    const { rows: settingsRows } = await pool.query(`SELECT service_requests_enabled FROM kiosk_settings WHERE location_id = $1`, [locationId]);
    if (settingsRows[0] && settingsRows[0].service_requests_enabled === false) {
      return res.status(403).json({ error: "This feature isn't enabled for this location." });
    }
    const request = {
      id: crypto.randomUUID(),
      locationId,
    ...
    broadcastServiceRequest(request);
    res.json({ ok: true });
  } catch (err) {
...
// Waitress marks an alert as seen/being handled — request stays visible on
// the station screen (so it isn't forgotten) but no longer reads as brand new.
app.post("/api/service-request/:locationId/:id/acknowledge", requireStation, (req, res) => {
  const request = serviceRequestsByLocation.get(req.params.locationId)?.get(req.params.id);
  if (!request) return res.status(404).json({ error: "Request not found" });
  request.status = "acknowledged";
  broadcastToStations(req.params.locationId, { type: "request_updated", request });
  res.json({ ok: true });
});

// Waitress marks an alert as done — removed from the station's list entirely.
app.post("/api/service-request/:locationId/:id/complete", requireStation, (req, res) => {
  const requests = serviceRequestsByLocation.get(req.params.locationId);
  if (!requests?.has(req.params.id)) return res.status(404).json({ error: "Request not found" });
  requests.delete(req.params.id);
  broadcastToStations(req.params.locationId, { type: "request_completed", requestId: req.params.id });
  res.json({ ok: true });
});

// The waitress station screen itself — separate password from /admin (see
// requireStation above), so servers can acknowledge/complete alerts without
// getting access to menu editing, QR codes, or anything else in /admin.
app.get("/waitress-station", requireStation, (req, res) => {
  res.sendFile(__dirname + "/private/waitress-station.html");
});
...
server.listen(PORT, () => {
  ...
  console.log(`  Waitress station:      http://localhost:${PORT}/waitress-station?location=<id>`);
