...
app.get("/api/kiosk-config/:locationId", requireDb, async (req, res) => {
  try {
    const [{ rows: ads }, { rows: settingsRows }] = await Promise.all([
      ...
      pool.query(`SELECT idle_timeout_seconds, ad_interval_seconds, ordering_enabled FROM kiosk_settings WHERE location_id = $1`, [
        req.params.locationId,
      ]),
    ]);
...
app.get("/api/table-order-check/:locationId", requireDb, async (req, res) => {
  try {
    const [allowed, { rows: settingsRows }] = await Promise.all([
      ...
      pool.query(`SELECT ordering_enabled FROM kiosk_settings WHERE location_id = $1`, [req.params.locationId]),
    ]);
    const orderingEnabled = settingsRows[0] ? settingsRows[0].ordering_enabled !== false : true;
    res.json({ allowed, orderingEnabled });
  } catch (err) {
...
app.post("/api/admin/menu/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO menu_items (location_id, category, name, price, description, modifiers, active, sort_order, image_url, available_togo)
...
app.put("/api/admin/menu/item/:id", requireDb, requireAdmin, express.json(), async (req, res) => {
  const { category, name, price, description, modifiers, active, sortOrder, imageUrl, availableTogo } = req.body;
  const { rows } = await pool.query(
    `UPDATE menu_items SET category=$1, name=$2, price=$3, description=$4, modifiers=$5, active=$6, sort_order=$7, image_url=$8, available_togo=$9
     WHERE id=$10 RETURNING *`,
    [category || "Menu", name, price ?? null, description || null, JSON.stringify(modifiers || []), active !== false, sortOrder || 0, imageUrl || null, availableTogo !== false, req.params.id]
  );
...
app.put("/api/admin/kiosk-settings/:locationId", requireDb, requireAdmin, express.json(), async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO kiosk_settings (location_id, idle_timeout_seconds, ad_interval_seconds, header_button_image_url, header_button_link_url, ordering_enabled)
     VALUES ($1,$2,$3,$4,$5,$6)
     ON CONFLICT (location_id) DO UPDATE SET
       idle_timeout_seconds=$2, ad_interval_seconds=$3, header_button_image_url=$4, header_button_link_url=$5, ordering_enabled=$6
     RETURNING *`,
    [req.params.locationId, idleTimeoutSeconds, adIntervalSeconds, headerButtonImageUrl, headerButtonLinkUrl, orderingEnabled]
  );
  res.json(rows[0]);
});
