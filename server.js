...
function normalizePosOrder(raw) {
  // Case 0 (preferred): "Entire payload" from Wix Studio Automations — the
  // real, un-mapped order object. This is the clean path; everything below
  // is legacy fallback from earlier experiments with custom body params.
  if (raw.businessLocation || (raw.orderId && Array.isArray(raw.lineItems))) {
    const locationId =
      raw.businessLocation?.id || slugify(raw.businessLocation?.name || "unknown-location");
    const items = (raw.lineItems || []).map((li) => ({
      name: li.name || li.title?.en_US || li.itemName || "Item",
      quantity: li.quantity || 1,
      modifiers: normalizeModifiers(li.modifiers ?? li.options ?? li.choices ?? li.selectedOptions),
...
function buildOrder({ orderId, locationId, createdAt, items, data, raw }) {
  const customer =
  ...
  return {
    ...
    items,
    notes: data.comment || data.checkoutAdditionalFields || null,
    customer,
    deliveryType: null,
  };
}
