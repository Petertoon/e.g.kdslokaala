...
function normalizePosOrder(raw) {
  const items = (raw.lineItems || raw.items || []).map((li) => ({
    name: li.title?.en_US || li.name || "Item",
    quantity: li.quantity || 1,
    modifiers: (li.modifiers || []).map((m) => m.title?.en_US || m.name).filter(Boolean),
    notes: li.comment || li.notes || null,
  }));

  return {
    id: raw.id || raw.orderId || String(Date.now()),
    locationId: raw.locationId || raw.businessLocationId || null,
    locationName: raw.locationName || raw.location?.name || null,
    placedAt: raw.dueDate || raw.createdDate || new Date().toISOString(),
    items,
    notes: raw.comment || null,
    customer: raw.contact?.name || raw.customer?.name || null,
